...
description: >
  USER-INVOKED ONLY. Use when the user explicitly invokes $looper or asks to
  design, audit, review, or convert a task into a Codex loop: design a loop,
  make this a loop, turn this into a Codex loop, Theo loop, Pete loop,
  loopify, inner loop, watch this PR, loop until review passes, or create a
  dynamic workflow. Do not invoke for normal implementation, review, or
  planning unless the user asks for a loop.
license: MIT
...
enforces it: a runner enforces it in code; a `/goal` payload merely states it
and relies on the executing agent honoring it. Every design names which of
...
- Design only. The deliverable ends at the emitted payload, spec, or
  ...
  actual Codex goal, and do not start executing a designed loop, even when
  asked to "just run it" — emit, then hand over.
- The user owns the loop's shape, bounds, and side effects. A number enters a
  design in exactly two ways: the user set it during the dialogue, or it
...
- Emission happens only after the user answers ok to a recap (dialogue step
  3). An amendment reopens the dialogue instead.

## Side-effect classes

Classify every loop by the strongest side effect it can cause; the class
appears in the recap and in the emitted artifact.

| Class | Meaning |
| --- | --- |
| Read-only | research, planning, PR/CI/issue inspection, local status checks |
| Local artifact | writes `.agents/loops/*.md` or requested planning artifacts |
...
| Merge/deploy/system | merges, deploys, installs, credentials, external systems |
...
3. **Recap and wait.** Restate the agreed loop in one compact block: shape,
   ...
   their enforcement mechanism, validation evidence, stop and pause
   conditions. End with exactly one question: ok to emit?
...
settled: Context / Constraints / Loop structure / Gates / Validation / Stop
condition / Pause condition / Allowed side effects / Forbidden side effects.
The payload states that its bounds are behavioral commitments, not enforced
limits. Over 4000 characters: read `goalify/SKILL.md`, follow its protected
goal-file procedure, and return the `/goal .agents/goals/<slug>.md` usage.
...
deep-modules `ralf-loop` runner: state file, phase sequence, fresh
implementer per unit, reviewer sidecars, feedback pass back to the
implementer, validation commands, JSONL run/agent ledgers, human gates as
prompts in the driver. The spec fixes the workflow policy (units, prompts,
...
loop structure, gates, bounds with enforcement, validation, stop/pause,
audit-log expectations. Sections the dialogue never touched do not appear.
...
- About to emit a payload, spec, or artifact with no ok'd recap above it →
  back to step 3.
- A number in the design the user never set or confirmed → mark it
  `<user sets>`.
